...
function getTimeStr(timestamp: number) {
  ...
  return `${startDate.getFullYear()}${(startDate.getMonth() + 1).toString().padStart(2, '0')}${startDate.getDate().toString().padStart(2, '0')}_${startDate.getHours().toString().padStart(2, '0')}${startDate.getMinutes().toString().padStart(2, '0')}${startDate.getSeconds().toString().padStart(2, '0')}`
}
/** 获取当次记录状态
...
class SaveInfo implements SaveInfoConfig {
  /** 文件id */
  get fileId() {
    return `${this.platform}_${this.room}-${getTimeStr(this.timestamp || 0)}${this.sliceByStatus ? getRecordStatus(this.status, this.statusChanged) : ""}${this.part ? `.${this.part}` : ""}`
  }
  /** 平台 */
  platform: string
  /** 房间号 */
  ...
  /** 分块id */
  part: number
  constructor({platform, room, status, timestamp, statusChanged, sliceByStatus = false, sliceByCount = 0}: SaveInfoConfig & { sliceByStatus?: boolean, sliceByCount?: number }) {
...
class MsgSave {
  ...
  write(message: any, {platform, room}: {platform: string, room: string | number}) {
    if (this.paused) return
    const roomKey = `${platform}:${room}`
    // 获取保存信息，若信息不存在，则设置一个信息
    const saveInfo = this.list.get(roomKey) || this.createSaveInfo({platform, room})
    // 若文件不存在，则创建一个有record_info信息的.floatrec文件
    if (!saveInfo.exist) this.createFile(roomKey)
    const file = path.join(this.path, `${this.prefix}${saveInfo.fileId}${this.suffix}.floatrec`)
    fs.writeFile(
      file,
    ...
    if (saveInfo.sliceByCount && saveInfo.sliceByCount <= saveInfo.count) saveInfo.slice()
  }
  start() {
    this.paused = false
  }
  ...
  /** 设置保存信息 */
  setSaveInfo({platform, room, status, timestamp, statusChanged}: SaveInfoConfig) {
  ...
  createSaveInfo({platform, room}: {platform: string, room: string | number}) {
    const {status, timestamp} = this.main.room.get(`${platform}:${room}`)?.info || {status: RoomStatus.off, timestamp: 0}
