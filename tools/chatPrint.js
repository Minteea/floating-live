...
class chatPrint {
    ...
    /** 获取特权粉丝名称 */
    getPriviliegeName(platform, level) {
        switch (platform) {
            case "bilibili":
    ...
    log(message) {
        ...
        switch (message.type) {
            ...
            case "privilege": {
                ...
                console.log(`${user}${"\x1b[1;31m"} ${action}了主播的${"\x1b[33m"}${name}${"\x1b[0m"}`);
                break;
            }
            case "superchat": {
                let user = this.getUserInfo(message);
                let second = Math.round(message.info.duration / 1000);
                console.log(`${"\x1b[1;36m"}SC(${second}s) ${user}${"\x1b[0m"}: ${message.info.text}`);
                break;
            }
            case "entry": {
                let user = this.getUserInfo(message);
            ...
            case "live_cut": {
                ...
                console.log(`${"\x1b[1;31m"}直播间${"\x1b[40;33m"} ${roomKey} ${"\x1b[1;31m"}被管理员切断${"\x1b[0m"}: ${msg}`);
                break;
            }
        }
    }
}
exports.default = chatPrint;
